...
export default function StatBar({stat}: StatBarProps) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      */}
      <span className="text-pokemon-gray w-16 flex-shrink-0 text-right text-xs">{statName}</span>

      {/* Stat value */}
      <span className="text-pokemon-black w-7 flex-shrink-0 text-xs font-bold">{stat.base_stat}</span>
      ...
      */}
      <div className="bg-pokemon-lightgray h-2 flex-1 overflow-hidden rounded-full">
        {/*
          The fill bar.
