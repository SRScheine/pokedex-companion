...
export default function SpinWheel() {
  ...
  */
    ...
    <div className="flex flex-col items-center gap-6">
      ...
      */}
      <div className="flex flex-wrap gap-2 justify-center w-full">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-pokemon-yellow text-pokemon-black rounded-full font-bold text-sm hover:bg-yellow-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
        >
        ...
        <button
          onClick={addRandom}
          disabled={isSpinning}
          className="flex items-center gap-2 px-4 py-2 bg-pokemon-blue text-white rounded-full font-bold text-sm hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
        >
          🎲 Random
        </button>
      </div>
      ...
      */}
      <div className="relative">
        ...
        */}
        ...
        {pokemon.length === 0 && !isLoadingFavorites && (
          ...
          */
          <div className="absolute inset-0 flex items-center justify-center rounded-full bg-pokemon-lightgray">
            <p className="text-pokemon-gray text-sm text-center px-8">
              Add some Pokémon to spin!
            </p>
          </div>
        )}
      </div>
      ...
      <button
        ...
        */
        className={`
          px-12 py-4 rounded-full font-[family-name:var(--font-pixel)] text-lg shadow-xl
          transition-all duration-150 active:scale-95
          ${canSpin
            ? "bg-pokemon-red text-white hover:bg-pokemon-darkred"
  ...
  <p className="text-xs text-pokemon-gray uppercase tracking-wide font-medium">
    On the wheel ({pokemon.length}/{MAX_SLOTS})
  </p>
  <button
  onClick={() => setPokemon([])}
  disabled={isSpinning}
  className="text-xs text-pokemon-gray border border-pokemon-lightgray rounded-full px-3 py-1 hover:border-pokemon-red hover:text-pokemon-red transition-colors disabled:cursor-not-allowed"
>
  Clear all
</button>
</div>
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {pokemon.map((p) => (
              <div
                ...
                */
                className="flex items-center gap-1.5 bg-white rounded-full pl-1 pr-3 py-1 shadow-sm border border-pokemon-lightgray"
              >
                <Image src={p.sprite} width={28} height={28} alt={p.name} unoptimized />
                <span className="text-xs font-medium capitalize text-pokemon-black">
                  {capitalize(p.name)}
                </span>
                <button
                  onClick={() => removePokemon(p.id)}
                  disabled={isSpinning}
                  className="text-pokemon-gray hover:text-pokemon-red transition-colors text-xs ml-0.5 disabled:cursor-not-allowed"
        ...
        <div className="w-full max-w-md">
          <div className="relative">
            ...
            <input
              ...
              disabled={isSpinning}
              className="w-full pl-10 pr-4 py-2.5 rounded-full border border-pokemon-lightgray bg-white focus:outline-none focus:ring-2 focus:ring-pokemon-red text-pokemon-black placeholder:text-pokemon-gray text-sm disabled:opacity-50"
            />
          </div>

          {isSearching && (
            <div className="flex items-center gap-2 py-3 px-2 text-pokemon-gray text-sm">
              {/*
          ...
          {searchResults.length > 0 && (
            ...
            */
            <div className="mt-2 bg-white rounded-2xl shadow-lg border border-pokemon-lightgray overflow-hidden">
              {searchResults.map((result) => {
                ...
                return (
                  <button
                    ...
                    */
                    className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-pokemon-lightgray transition-colors text-left disabled:opacity-50 disabled:cursor-not-allowed border-b border-pokemon-lightgray last:border-0"
                  >
                    <Image
                      src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${result.id}.png`}
                      width={32} height={32} alt={result.name} unoptimized
                    />
                    <span className="text-sm font-medium capitalize text-pokemon-black flex-1">
                      {capitalize(result.name)}
                    </span>
                    <span className="text-xs text-pokemon-gray">
                      {formatPokemonId(result.id)}
                    </span>
                    {onWheel && (
                      <span className="text-xs text-pokemon-red font-medium">On wheel</span>
                    )}
                  </button>
                );
              })}
      ...
      {isFull && (
        <p className="text-xs text-pokemon-gray text-center">
          Wheel is full! Remove a Pokémon to add another.
        </p>
      )}
